  ...
  <div class="ui container" style="min-height:100%">
    ...
    <div class="ui link four doubling cards" v-else>
      <div
        ...
        :key="show.id"
        @click="$router.push({ name: 'shows', params: { id: show.id, show } })"
      >
        <div class="content">
          ...
          <div class="meta">
            ...
            <i class="film icon" v-if="show.trailer_url"></i>
          </div>
        </div>
        <div class="image">
...
export default {
  ...
  async mounted() {
    ...
    await this.$store.dispatch(
      "fetchShows",
      "https://astral.ctcd.org/api/shows"
    );
    this.loading = false;
  },
  computed: {
    ...
    filteredShows() {
      ...
      if (this.name != "")
        filteredShows = filteredShows.filter(s =>
          s.name.toLowerCase().includes(this.name.toLowerCase())
        );
